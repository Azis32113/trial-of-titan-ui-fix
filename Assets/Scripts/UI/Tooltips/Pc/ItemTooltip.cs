...
namespace Pc
{
    public class ItemTooltip : PcTooltip<Item>, IDescriber
    {
        ...
        protected override void Load(Player player, bool owned, Item obj)
        {
            ...
            itemImage.material = MaterialManager.GetUIMaterial(obj.enchantLevel);
        ...
        {
            AddContent($"<color=#{ColorUtility.ToHtmlStringRGB(color)}>{content}</color>");
            justAddedLine = false;
        }
        ...
        public void AddElement(string element, Color color)
        {
            AddElement($"<color=#{ColorUtility.ToHtmlStringRGB(color)}>{element}</color>");
        }
